...
fs = 5
dataset = TemporalChunkedDataset("data/time_series/indrani_zeta_ca_h_std_norm.pickle", 
                                   standardize_inputs=False, min_max_scale=False, time_chunk_size=40, steps=fs)
...
model = train_temporal_transformer(dataset=train_dataset, n_rates = dataset.n_rates, hidden_dim=128, 
                           output_dim=dataset.input_size, nEpochs=50, batch_size=100)
tc.save(model, 'model/indrani_transformer_chunked' + str(int(fs)) + '.pt')
...
plot_scatter(truth, predictions, output="transformer")
...
for r in range(6):
    for c in range(5):
        ...
        i+=1
plt.savefig("transformer_chunk_validation_ixr_est_"+ str(fs) + ".png")
...
plt.savefig("TransformerSanityCheckChunk" +str(fs) + ".png")
