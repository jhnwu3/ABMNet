...
for r in range(6):
    for c in range(5):
        ...
        axes[r,c].plot(indrani_estimates.times[fs:], predictions.cpu().detach().numpy(), c='orange', label="predictions")
        axes[r,c].plot(indrani_estimates.times[fs:], outputs.cpu().detach().numpy(), c='blue', label="truth")
        i+=1
plt.legend()
...
prediction = model(pseudoRates.to(device), input.to(device))
plt.figure()
