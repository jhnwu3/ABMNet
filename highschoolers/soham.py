...
if __name__ == '__main__':
    ...
    csv_file = "data/static/l3p/l3p_t3.csv"
    ...
    train_size = int(0.75 * len(abm_dataset))
    ...
    epoch_combos = [10, 20, 30, 40 ,50, 100, 150]
    best_n_epochs = 25
    ...
    for i in range(1, 6):
        combos.append([i])
        for j in range(0, 4):
            combos[i-1].append(8 * (2**j))
    combos = pd.DataFrame(combos)
    errors = [] #Will hold specific d, h, and mse values
    for k in range(0, len(epoch_combos)):
        for i in range(0, len(combos)):
            for j in range(1, 2): #len(combos[0])
            # now train using whatever cross-validated or user-specified
                if model_type == 'res_nn':
                    ABMNet = train_res_nn(train_dataset, input_size=input_len, hidden_size=combos[0][i], depth=combos[j][i], output_size=output_len, nEpochs=epoch_combos[k], use_gpu=using_GPU)
                else:
                    ABMNet = train_nn(train_dataset, input_size=input_len, hidden_size=combos[0][i], depth=combos[j][i], output_size=output_len, nEpochs=epoch_combos[k], use_gpu=using_GPU)
                if saving_model:
                ...
                errors.append([combos[0][i], combos[j][i], mse, epoch_combos[k]])
    ...
    errors.set_axis(labels = ["H", "D", "MSE", "Epochs"], axis = "columns", inplace = True)
    print(errors)
    errors.to_csv("soham_df_errors.csv")

    filter_data = errors[errors["H"] == 5]
    #filter_data = filter_data[filter_data["D"] == 64]
    print(filter_data)
    
    linReg = LinearRegression()
    linReg.fit(np.array(filter_data["Epochs"]).reshape((-1, 1)), np.array(filter_data["MSE"]))
    r_sq = linReg.score(np.array(filter_data["Epochs"]).reshape((-1, 1)), np.array(filter_data["MSE"]))
    print(r_sq)
    
    plt.scatter(x = filter_data["Epochs"], y = filter_data["MSE"])
    plt.plot(filter_data["Epochs"],linReg.predict(np.array(filter_data["Epochs"]).reshape((-1, 1))), color='r', label='Regression Line')
    plt.title("Given that H = 5 and D = 64, relationship between Epochs and MSE")
    plt.xlabel("# of Epochs")
    plt.ylabel("MSE (Error measure)")
    # plt.show()
    plt.savefig("SohamEpochsGraph.png")



    #result = errors.pivot(index="H",columns='D',values='MSE')
